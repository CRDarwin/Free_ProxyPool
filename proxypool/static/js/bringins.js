    ...
    i.fn.bringins = function (n) {
        ...
        if (n = i.extend(t, n), 0 == running) {
            ...
            count++, active_list[count] = current, n.width.indexOf("%") >= 0 ? (temp = n.width.replace("%", "") / 100, bringins_width = temp * innerWidth, bringins_margin = innerWidth - bringins_width) : (bringins_width = n.width, bringins_margin = innerWidth - bringins_width), "left" == n.position && i(this).css({
                ...
                position: "fixed",
                top: "0%"
            }), "" != n.zIndex && i("#bringins-block-" + count).css({"z-index": n.zIndex}), i("#bringins-block-close-" + count).css({
                fill: "white",
                ...
                "-moz-transition": "1s",
                "-webkit-transition": "1s"
            }), "left" == n.position && i("#bringins-svg-" + count).css({transform: "rotate(180deg)"}), mm_temp = innerWidth - 50, "right" == n.position && (i("#bringins-svg-" + count).css({
                top: "50%",
                ...
                "center" == n.position ? (i("#bringins-svgpath-" + count).css({transform: "scale(" + temp_temp + ",3)"}), i("#bringins-svg-" + count).css({left: mm_center_temp})) : (i("#bringins-svgpath-" + count).css({transform: "scale(1," + innerHeight + ")"}), i("#bringins-svg-" + count).css({top: "0%"}))
            }, 1), setTimeout(function () {
                i("#bringins-svgpath-" + count).css({transform: "scale(" + bringins_width + "," + innerHeight + ")"}), i(current_obj).fadeIn(1), "right" == n.position && i(current_obj).css({left: bringins_margin}), "left" == n.position && i(current_obj).css({left: 0}), "center" == n.position && (i("#bringins-svg-" + count).css({top: "0%"}), i(current_obj).css({top: "25px"}))
